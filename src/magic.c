...
void say_spell( CHAR_DATA *ch, int sn )
{
    ...
    {
	for ( iSyl = 0;
	     ( length = strlen( syl_table[iSyl].old_s ) ) != 0;
	     iSyl++ )
	{
	    if ( !str_prefix( syl_table[iSyl].old_s, pName ) )
	    {
		strcat( buf, syl_table[iSyl].new_s );
		break;
	    }
	}
    ...
    {
	if ( rch != ch )
	    act( ch->cclass==rch->cclass ? buf : buf2, ch, NULL, rch, TO_VICT );
    }
...
bool saves_spell( int level, CHAR_DATA *victim, int dam_type )
{
    ...
    if ( CHECK_IMM( victim, RIS_MAGIC ) )
      return TRUE;

    modifier = 10;
    if ( CHECK_RES( victim, dam_type ) )
	modifier -= 2;
    if ( CHECK_IMM( victim, dam_type ) )
	modifier -= 10;
    if ( CHECK_SUS( victim, dam_type ) )
	modifier += 2;
    if ( modifier <= 0 )
	return TRUE;
    if ( modifier != 10 )
      save = (save * modifier) / 10;
...
void spell_acid_blast( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    dam = dice( level, 8 );
    if ( saves_spell( level, victim, RIS_ACID ) )
	dam /= 2;
...
void spell_blindness( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_HITROLL;
    af.modifier  = -4;
    af.bitvector = new_xbv( AFF_BLIND, -1 );
...
void spell_breathe_water( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_GILLS, -1 );
...
void spell_burning_hands( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( saves_spell( level, victim, RIS_FIRE ) )
	dam /= 2;
...
void spell_call_lightning( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( vch->in_room == ch->in_room )
	{
	    ...
	    {
                damage( ch, vch, saves_spell( level, vch, RIS_LIGHT )
		       ? dam/4 : dam/2, sn, WEAR_NONE, DAM_LIGHT );
                damage( ch, vch, saves_spell( level, vch, RIS_ELECTRICITY )
...
void spell_change_sex( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.bitvector = new_xbv( AFF_CHANGE_SEX, -1 );
...
void spell_charm_person( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	|| level < victim->level
	|| saves_spell( level, victim, RIS_CHARM ) )
	return;
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_CHARM, -1 );
...
void spell_chill_touch( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    level    = UMAX( 0, level );
    dam	     = number_range( dam_each[level] / 2, dam_each[level] * 2 );
    if ( !saves_spell( level, victim, RIS_COLD ) )
    {
	af.type      = sn;
	af.duration  = 6;
...
void spell_colour_spray( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    level    = UMAX( 0, level );
    dam	     = number_range( dam_each[level] / 2, dam_each[level] * 2 );
    if ( saves_spell( level, victim, RIS_LIGHT ) )
	dam /= 2;
...
void spell_curse( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( IS_AFFECTED( victim, AFF_CURSE ) || saves_spell( level, victim, RIS_BLACK_MANA ) )
    {
	send_to_char( "You have failed.\n\r", ch );
	return;
    }
    ...
    af.location  = APPLY_HITROLL;
    af.modifier  = -1;
    af.bitvector = new_xbv( AFF_CURSE, -1 );
...
void spell_detect_evil( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_DETECT_EVIL, -1 );
...
void spell_detect_magic( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_DETECT_MAGIC, -1 );
...
void spell_dispel_evil( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    dam = dice( level, 4 );
    if ( saves_spell( level, victim, RIS_WHITE_MANA ) )
	dam /= 2;
...
void spell_dispel_magic ( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	for ( paf = victim->affected; paf; paf = paf->next )
	{
	    if ( paf->deleted )
	        continue;
	    if ( !saves_spell( level, victim, RIS_MAGIC ) )
	    {
	...
	if ( IS_AFFECTED( victim, AFF_SANCTUARY)
	    && !saves_spell( level, victim, RIS_MAGIC ) )
	{
...
void spell_energy_drain( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( saves_spell( level, victim, RIS_BLACK_MANA ) )
    {
...
void spell_faerie_fire( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_AC;
    af.modifier  = effect;
    af.bitvector = new_xbv( AFF_FAERIE_FIRE, -1 );
...
void spell_faerie_fog( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( ich == ch || saves_spell( level, ich, RIS_MAGIC ) )
	    continue;
...
void spell_fly( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_FLYING, -1 );
...
void spell_general_purpose( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( saves_spell( level, victim, RIS_PIERCE ) )
	dam /= 2;
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( obj->value[4] >= 0 && obj->value[4] < MAX_SPELL )
	{
	    send_to_char( " '", ch );
...
void spell_invis( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_INVISIBLE, -1 );
...
void spell_lightning_bolt( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    level    = UMAX( 0, level );
    dam	     = number_range( dam_each[level] / 2, dam_each[level] * 2 );
    if ( saves_spell( level, victim, RIS_ELECTRICITY ) )
	dam /= 2;
...
void spell_magic_missile( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    num = 1 + UMIN( 4, level / 4 );

    for ( i = 0; i < num; i++ )
    {
	dam = 1 + dice( 1, 4 );
	if ( saves_spell( level, victim, RIS_ENERGY ) )
	  dam /= 2;
...
void spell_mute( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	|| saves_spell( level, victim, RIS_BLACK_MANA ) )
	return;
    af.type      = sn;
    af.duration  = level;
    af.location  = 0;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_MUTE, -1 );
...
void spell_pass_door( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_PASS_DOOR, -1 );
...
void spell_plague( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( saves_spell( level, victim, RIS_DISEASE ) || IS_NPC( victim ) )
    {
        if ( ch == victim )
...
void spell_poison( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( saves_spell( level, victim, RIS_POISON ) )
	return;
    ...
    af.location  = APPLY_STR;
    af.modifier  = -2;
    af.bitvector = new_xbv( AFF_POISON, -1 );
...
void spell_polymorph_other( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.bitvector = new_xbv( AFF_POLYMORPH, -1 );
	...
	logln( "Log %s: casting Polymorph other", ch->name );
...
void spell_protection_evil( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_PROTECT_EVIL, -1 );
...
void spell_protection_good( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_PROTECT_GOOD, -1 );
...
void spell_remove_alignment( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( !xIS_SET( obj->extra_flags, ITEM_EVIL		)
    ...
    {
	xREMOVE_BIT( obj->extra_flags, ITEM_EVIL	 );
...
void spell_remove_curse( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( xIS_SET( obj->extra_flags, ITEM_NOREMOVE ) )
	{
...
void spell_sanctuary( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_SANCTUARY, -1 );
...
void spell_sleep( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_SLEEP, -1 );
...
void spell_summon( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	|| ( IS_NPC( victim ) && saves_spell( level, victim, RIS_MAGIC ) )
	|| IS_SET( victim->in_room->area->act,      AREA_PROTO ) )
    {
	send_to_char( "You failed.\n\r", ch );
	return;
    }
...
void spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	|| ( victim != ch
	    && ( saves_spell( level, victim, RIS_MAGIC )
		|| saves_spell( level, victim, RIS_MAGIC ) ) )
	|| IS_SET( pRoomIndex->area->act, AREA_PROTO ) )
    {
	send_to_char( "You failed.\n\r", ch );
	return;
    }
...
void spell_ventriloquate( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( !is_name( speaker, vch->name ) )
	    send_to_char( saves_spell( level, vch, RIS_SOUND ) ? buf2 : buf1, vch );
    }
...
void spell_weaken( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( is_affected( victim, sn ) || saves_spell( level, victim, RIS_BLACK_MANA ) )
	return;
...
void spell_acid_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( number_percent( ) < 2 * level && !saves_spell( level, victim, RIS_ACID ) )
    {
...
void spell_fire_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( number_percent( ) < 2 * level && !saves_spell( level, victim, RIS_FIRE ) )
    {
	for ( obj_lose = victim->carrying; obj_lose; obj_lose = obj_next )
	{
	    char *msg;
...
void spell_frost_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( number_percent( ) < 2 * level && !saves_spell( level, victim, RIS_COLD ) )
    {
	for ( obj_lose = victim->carrying; obj_lose; obj_lose = obj_next )
	{
	    char *msg;
...
void spell_gas_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( IS_NPC( ch ) ? !IS_NPC( vch ) : IS_NPC( vch ) )
	{
	    hpch = UMAX( 10, ch->hit );
	    dam  = number_range( hpch / 8 + 1, hpch / 4 );
	    if ( saves_spell( level, vch, RIS_POISON ) )
		dam /= 2;
...
void spell_awe ( int sn, int level, CHAR_DATA *ch, void *vo )
  {
    ...
    if ( victim->fighting == ch && !saves_spell( level, victim, RIS_MENTAL ) )
    {
	stop_fighting ( victim, TRUE);
...
void spell_ballistic_attack ( int sn, int level, CHAR_DATA *ch, void *vo )
  {
    ...
    level    = UMAX( 0, level );
    dam	     = number_range( dam_each[level] / 2, dam_each[level] * 2 );
    if ( saves_spell( level, victim, RIS_BASH ) )
      dam /= 2;
...
void spell_death_field ( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( !IS_NPC( ch ) ? IS_NPC( vch ) : IS_NPC( vch ) )
	{
	    ...
	    if ( !saves_spell( level, vch, RIS_BLACK_MANA )
		&& (   level <= vch->level + 5
		    && level >= vch->level - 5 ) )
            {
...
void spell_disintegrate ( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	&& !saves_spell( level, victim, RIS_ENERGY ) )
      for ( obj_lose = victim->carrying; obj_lose; obj_lose = obj_next )
      {
	  obj_next = obj_lose->next_content;
	  if ( obj_lose->deleted )
...
void spell_ego_whip ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( is_affected( victim, sn ) || saves_spell( level, victim, RIS_MENTAL ) )
        return;
...
void spell_psionic_blast ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    level    = UMAX( 0, level );
    dam	     = number_range( dam_each[level] / 2, dam_each[level] * 2 );
    if ( saves_spell( level, victim, RIS_MENTAL ) )
        dam /= 2;
...
void spell_ultrablast ( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( IS_NPC( ch ) ? !IS_NPC( vch ) : IS_NPC( vch ) )
	{
	    hpch = UMAX( 10, ch->hit );
	    dam  = number_range( hpch / 8+1, hpch / 4 );
	    if ( saves_spell( level, vch, RIS_MENTAL ) )
	        dam /= 2;
...
void spell_vampiric_bite( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( saves_spell( level, victim, RIS_DISEASE )
	|| number_bits( 1 ) == 0 )
	return;
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_VAMP_BITE, -1 );
...
void spell_turn_undead( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	|| saves_spell( level, victim, RIS_WHITE_MANA ) )
    {
        send_to_char( "You have failed.\n\r", ch );
	return;
    }
...
void spell_meteor_swarm( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( vch->in_room == ch->in_room )
	{
	    if ( vch != ch && ( IS_NPC( ch ) ? !IS_NPC( vch )
			                     :  IS_NPC( vch ) ) )
	    {
    		if ( saves_spell( level, vch, RIS_BASH ) )
        		dam /= 2;
...
void spell_chain_lightning( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	for ( vch = char_list; vch; vch = vch->next )
	{
	    ...
	    if ( vch->in_room == ch->in_room )
	    {
		if ( !saves_spell( level, vch, RIS_ELECTRICITY ) && vch != vch_last )
		{
		    found = TRUE;
		    vch_last = vch;
		    ...
		    dam = dice( level, 6 );
		    if ( saves_spell( level, vch, RIS_ELECTRICITY ) )
			dam /= 3;
	...
	if ( !found )
	{
	  ...
	  dam = dice( level, 6 );
	  if ( saves_spell( level, ch, RIS_ELECTRICITY ) )
	    dam /= 3;
...
void spell_detect_good( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = new_xbv( AFF_DETECT_GOOD, -1 );
...
void spell_flame_shield( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	&& !saves_spell( level, victim, RIS_FIRE ) )
    {
    ...
    af.location  = APPLY_NONE;
    af.bitvector = new_xbv( AFF_FLAME_SHIELD, -1 );
    affect_to_char( victim, &af );
...
void spell_frost_shield( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	&& !saves_spell( level, victim, RIS_COLD ) )
    {
    ...
    af.location  = APPLY_NONE;
    af.bitvector = new_xbv( AFF_FROST_SHIELD, -1 );
    affect_to_char( victim, &af );
...
void spell_shock_shield( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.location  = APPLY_NONE;
    af.bitvector = new_xbv( AFF_SHOCK_SHIELD, -1 );
    affect_to_char( victim, &af );
...
/*
 * Code for Jester spells/skills by TFC <tfco@rnl.ist.utl.pt>
 */
void spell_acid_tomato( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA   *victim = (CHAR_DATA *) vo;
    AFFECT_DATA  af;
    int          effect;

    if ( number_percent( ) > 50 + ch->level * 3 - victim->level * 3 )
    {
        send_to_char( "Your acid tomato misses the targuet.\n\r", ch );
	return;
    }

    effect       = UMIN( level, 20 );
    ...
    act( "A flying tomato hits $n .", victim, NULL, NULL, TO_ROOM );
    send_to_char( "You feel your skin burning.\n\r", victim );
    return;
}


void spell_life_drain( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int        dam;
    ...
    if ( saves_spell( level, victim, RIS_BLACK_MANA ) )
    {
	send_to_char( "You failed to drain your victim's life.\n\r", victim );
	return;
    }
...
void spell_potato( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA  *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
	...
	|| saves_spell( level, victim, RIS_BLACK_MANA ) )
    {
	send_to_char( "Your potato misses the target.\n\r", ch );
	return;
    }
    af.type      = sn;
    af.duration  = level;
...
void spell_cream_pie( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( number_percent( ) > 25 + ch->level * 3 - victim->level * 3 )
    {
	send_to_char( "Your pie misses the target.\n\r", ch );
	return;
    }
...
void spell_hypnotize( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA   *victim = (CHAR_DATA *) vo;
    AFFECT_DATA  af;
    ...
    if ( victim == ch )
    {
	send_to_char( "You feel stupid!\n\r", ch );
	return;
    }

    if ( get_trust( ch ) > LEVEL_HERO && get_trust( ch ) < L_SEN )
    {
	send_to_char( "There is no need for you to hypnotize mobiles\n\r",
		     ch );
	logln( "%s hypnotizing", ch->name );
	return;
    }
    ...
    if ( saves_spell( level, victim, RIS_CHARM ) )
	if ( saves_spell( level, victim, RIS_CHARM ) )
	return;
...
void spell_sing_lullaby( int sn, int level, CHAR_DATA *ch, void *vo )
{

    CHAR_DATA  *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;

    if ( IS_NPC( ch ) || number_percent( ) > ch->pcdata->learned[gsn_sing] )
    {
	send_to_char( "You failed to sing.\n\r", ch );
	return;
    }
...
void spell_sing_war( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA   *gch;
    AFFECT_DATA  af;
	...
	send_to_char( "You feel confident.\n\r", gch );
    }
...
void spell_sing_hope( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *gch;
    AFFECT_DATA af;
...
void spell_sing_courage( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *gch;
    AFFECT_DATA af;
...
void spell_sing_opera( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *rch;
...
void spell_magic_leaf( int sn, int level, CHAR_DATA *ch, void *vo )
{
    OBJ_DATA *leaf;

    leaf = create_object( get_obj_index( OBJ_VNUM_LEAF ), 0 );

    leaf->value[0]	= 1;
    leaf->timer		= 1;
    obj_to_char( leaf, ch );

    act( "$p suddenly appears.", ch, leaf, NULL, TO_CHAR );
    return;
}
