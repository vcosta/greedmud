...
void do_asave( CHAR_DATA *ch, char *argument )
{
	...
	{
	    save_area( pArea );
	    REMOVE_BIT( pArea->act, AREA_CHANGED );
	    REMOVE_BIT( pArea->act, AREA_ADDED   );
	}
	...
	if ( !str_cmp( "verbose", argument ) )
	    SET_BIT( pArea->act, AREA_VERBOSE );

	save_area( pArea );
	REMOVE_BIT( pArea->act, AREA_VERBOSE );
	return;
    }
	...
	for ( pArea = area_first; pArea; pArea = pArea->next )
	{
	    ...
	    if (   IS_SET( pArea->act, AREA_CHANGED )
		|| IS_SET( pArea->act, AREA_ADDED   ) )
	    {
		if ( !str_cmp( "verbose", argument ) )
		    SET_BIT( pArea->act, AREA_VERBOSE );
