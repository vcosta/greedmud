...
void do_give( CHAR_DATA *ch, char *argument )
{
	...
	|| ( IS_NPC( victim ) && obj->item_type == ITEM_SCROLL )
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    if ( (         !IS_NPC( victim )
	  && ( (   !xIS_SET( victim->act, PLR_KILLER )
		...
		&& !xIS_SET( victim->act, PLR_REGISTER )
    ...
    if ( !can_drop_obj( ch, obj )
	|| xIS_SET( obj->extra_flags, ITEM_INVENTORY )
	|| obj->level > ch->level )
    {
 ...
 * By Aioros.
 */
void move_obj( OBJ_DATA *obj, int door )
{
    EXIT_DATA	    *pexit;
    ROOM_INDEX_DATA *in_room;
    ...
    {
	bugf( "move_obj: bad way %d.", door );
	return;
    }

    if ( IS_SET( to_room->area->act, AREA_PROTO ) )
    {
     	bugf( "move_obj: area under construction." );
     	return;
    }

    if ( IS_SET( pexit->exit_info, EX_CLOSED ) )
    {
     	bugf( "move_obj: closed door %d.", door );
     	return;
    }

    {
	ROOM_INDEX_DATA * inside;
     	char  	      	  buffer  [ MAX_STRING_LENGTH ];

	inside = get_room_index( obj->value[1] );

     	sprintf( buffer, "%s leaves %s.", obj->short_descr, dir_name[door] );
     	send_to_room( buffer, in_room );

     	sprintf( buffer, "You roll %s.", dir_name[door] );
     	send_to_room( buffer, inside );

     	obj_from_room( obj );
     	obj_to_room( obj, to_room );

     	sprintf( buffer, "%s has arrived.", obj->short_descr );
     	send_to_room( buffer, to_room );
    }
}
