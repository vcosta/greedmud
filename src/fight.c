...
void violence_update( void )
{
	...
	{
	    if ( IS_NPC( ch ) && !victim->deleted )
	    {
		if ( HAS_TRIGGER( ch, TRIG_FIGHT ) )
		   mp_raw_percent_trigger( ch, victim, NULL, NULL, TRIG_FIGHT );
...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, int wpn )
{
     ...
     */
    wpn_gsn	= gsn_hit;
    dam_type	= race_table[ch->race].dmg_type;
    if ( wield && wield->item_type == ITEM_WEAPON )
    {
    ...
    {
	thac0_00 = ch->cclass[0]->thac0_00;
	thac0_47 = ch->cclass[0]->thac0_47;
    }
 ...
 *					-Thoric
 */
int ris_damage( CHAR_DATA *ch, int dam, int dam_type )
{
    int modifier;
    int bit;
	    ...
	    int dam_type )
{
    ...
    dam = ris_damage( victim, dam, dam_type );
     ...
     */
    if ( dt != TYPE_UNDEFINED )
        dam_message( ch, victim, dam, dt, wpn, (dam == -1) );
	...
	if ( IS_AFFECTED( victim, AFF_FLAME_SHIELD )
	    && !IS_AFFECTED( ch, AFF_FLAME_SHIELD ) )
	    damage( victim, ch, dam/2, gsn_flame_shield, WEAR_NONE,
							 DAM_FIRE );

	if ( IS_AFFECTED( victim, AFF_FROST_SHIELD )
	    && !IS_AFFECTED( ch, AFF_FROST_SHIELD ) )
	    damage( victim, ch, dam/2, gsn_frost_shield, WEAR_NONE,
							 DAM_COLD );

	if ( IS_AFFECTED( victim, AFF_SHOCK_SHIELD )
	    && !IS_AFFECTED( ch, AFF_SHOCK_SHIELD ) )
	    damage( victim, ch, dam/2, gsn_shock_shield, WEAR_NONE,
							 DAM_ELECTRICITY );
    }
...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    {
	send_to_char(
	    "You don't see a reason for doing that.\n\r", ch );
	return TRUE;
    }
...
void check_killer( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( xIS_SET( victim->act, PLR_KILLER )
	|| xIS_SET( victim->act, PLR_THIEF )
	|| ( xIS_SET( ch->act, PLR_KILLER ) && ch->fighting )
...
bool check_shield_block( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( IS_NPC( victim ) )
	/* Zen was here.  :) */
        chance  = UMIN( 20, victim->level );
    else
	chance  = victim->pcdata->learned[gsn_shield_block] / 5 + 3;
...
void make_corpse( CHAR_DATA *ch )
{
	...
	xREMOVE_BIT( obj->extra_flags, ITEM_VIS_DEATH );
...
void raw_kill( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( IS_NPC( victim ) && HAS_TRIGGER( victim, TRIG_DEATH ) )
	    cry = !mp_raw_percent_trigger( victim, ch, NULL, NULL, TRIG_DEATH );
        else
	    cry = TRUE;
    }
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim )
{
    ...
    /* More exp at the low levels, less at high -Zen */
    xp     = interpolate( gch->level, xp, xp * 3 / 5 );
...
void do_tickles( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char       arg [ MAX_INPUT_LENGTH ];
    int        percent;
    ...
    /* Don't allow charmed mobiles to do this, check player's level */
    if ( ( IS_NPC( ch ) && IS_AFFECTED( ch, AFF_CHARM ) )
	|| !can_use( ch, gsn_tickles ) )
    {
	send_to_char( "You don't know how to tickle opponents.\n\r", ch );
	return;
    }
    ...
    if ( !victim->fighting )
    {
	act( "$E is not fighting anyone!", ch, NULL, victim, TO_CHAR );
	return;
    }
    ...
    WAIT_STATE( ch, skill_table[gsn_tickles].beats );
    percent = number_percent( ) + victim->level - ch->level;

    if ( get_eq_char( ch, WEAR_WIELD ) )
	percent *= 2;		/* 1/2 as likely when wielding a weapon */

    if ( IS_NPC( ch ) || percent < ch->pcdata->learned[gsn_tickles] * 2 / 3 )
    {
	disarm( ch, victim );
	learn( ch, gsn_tickles, TRUE );
    }
    else
    {
	send_to_char( "You failed.\n\r", ch );
	learn( ch, gsn_tickles, FALSE );
    }
...
void do_stake( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( race_table[victim->race].name, "Vampire" ) )
	{
	    ...
	    af.location  = APPLY_NONE;
	    af.modifier  = 0;
	    af.bitvector = new_xbv( AFF_GHOUL, -1 );
...
bool licensed ( CHAR_DATA *ch )
{
    ...
    if ( !xIS_SET( ch->act, PLR_REGISTER ) )
        return FALSE;
