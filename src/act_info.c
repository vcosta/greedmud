...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	|| IS_AFFECTED( victim, AFF_FROST_SHIELD )
	|| IS_AFFECTED( victim, AFF_SHOCK_SHIELD ) )
						strcat( buf, "{o{d(Black Aura){r " );
    if ( !IS_NPC( victim ) && xIS_SET( victim->act, PLR_KILLER )  )
						strcat( buf, "(KILLER) "     );
    if ( !IS_NPC( victim ) && xIS_SET( victim->act, PLR_THIEF  )  )
						strcat( buf, "(THIEF) "      );
    if ( !IS_NPC( victim ) && xIS_SET( victim->act, PLR_REGISTER  )  )
						strcat( buf, "(PK) "      );
...
void do_look( CHAR_DATA *ch, char *argument )
{
    ...
    {
	/* 'look' or 'look auto' */
	if ( !xIS_SET( ch->act, PLR_EDIT_INFO ) )
	{
	    sprintf ( buf, "{o{c%s{x\n\r", ch->in_room->name );
	    send_to_char( buf, ch );
	}
	...
	{
            sprintf ( buf, "{x%s{x", ch->in_room->description );
	    send_to_char( buf, ch );
	}
    ...
    if ( !str_prefix( arg1, "outside" ) )
    {
        ROOM_INDEX_DATA *original;

    	if ( !ch->in_room->vehicle )
    	{
    	   send_to_char( "You must be in a vehicle to do so.\n\r", ch );
    	   return;
    	}

    	original = ch->in_room;
    	char_from_room( ch );
    	char_to_room( ch, original->vehicle->in_room );
...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->level >= 10 )
	sprintf( buf, "%+4.4d, %-12.12s   ", ch->alignment, buf2 );
    else
	sprintf( buf, "%-18.18s    ", buf2 );
    strcat( buf1, buf );
    ...
    sprintf( buf, " Pos'n: %-19.19s  Weight: %5.5d (max %7.7d){x\n\r",
	    buf2, ch->carry_weight, can_carry_w( ch ) );
    strcat( buf1, buf );
    ...
    sprintf( buf,
	    "                                             Autosac [%c]{x\n\r",
	    ( !IS_NPC( ch ) && xIS_SET( ch->act, PLR_AUTOSAC  ) ) ? 'X'
	                                                          : ' ' );
    strcat( buf1, buf );
    ...
    sprintf( buf,
	    ...
	    ch->practice,
	    ch->hit,  ch->max_hit,
	    ( !IS_NPC( ch ) && xIS_SET( ch->act, PLR_PAGER    ) ) ? 'X'
	                                                          : ' ',
	    ( IS_NPC( ch ) ? 0 : ch->pcdata->pagelen ),
	    ( !IS_NPC( ch ) && xIS_SET( ch->act, PLR_AUTOEXIT ) ) ? 'X'
	                                                          : ' ' );
    strcat( buf1, buf );
    ...
    sprintf( buf,
	    ...
	    ( !IS_NPC( ch ) && xIS_SET( ch->act, PLR_AUTOLOOT ) ) ? 'X'
	                                                          : ' ' );
    strcat( buf1, buf );
    ...
    sprintf( buf,
	    ...
	    ( !IS_NPC( ch ) && xIS_SET( ch->act, PLR_AUTOGOLD ) ) ? 'X'
	                                                          : ' ' );
    strcat( buf1, buf );
...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( buf, ch );
    sprintf( buf,
	    "{o{yGreedMud started up %s\rThe system time is %s{x\r",
	    str_boot_time,
...
void do_who( CHAR_DATA *ch, char *argument )
{
	 ...
	 */
	cclass = class_short( wch );
	if ( wch->level >= LEVEL_IMMORTAL )
	    switch ( wch->level )
	      {
	      default: break;
	      case L_DIR: cclass = "Director";	 break;
	      case L_SEN: cclass = "Senior";	 break;
	      case L_JUN: cclass = "Junior";	 break;
	      case L_APP: cclass = "Apprentice"; break;
	      }
	...
	if ( wch->level < LEVEL_IMMORTAL )
	{
	    ...
	    sprintf( buf1, "%s%s%s%s%s",
		    xIS_SET( wch->act, PLR_KILLER   ) ? "{o{r(KILLER) {x" : "",
		    xIS_SET( wch->act, PLR_THIEF    ) ? "{o{r(THIEF) {x"  : "",
		    xIS_SET( wch->act, PLR_AFK      ) ? "{y[AFK] {x"    : "",
		    wch->name,
		    wch->pcdata->title );
	    ...
	    sprintf( buf2, " %c%s%c",
		    ...
		    rightletter );
	    sprintf( buf + strlen( buf ), "%-2d %-12s %s.%s{x\n\r",
		    wch->level,
		    cclass,
		    buf1,
		    buf2 );
	...
	else
	{
	    ...
	    sprintf( buf1, "%s%s%s%s",
		    xIS_SET( wch->act, PLR_WIZINVIS ) ? "{o{b(WIZINVIS) {x" : "", 
/*
...
*/
	    ...
	    sprintf( buf2, " %c%s%c",
		    ...
		    !is_clan( wch ) ? "" : wch->pcdata->clan->who_name,
		    rightletter );
	    sprintf( buf + strlen( buf ), "%-15s %s.%s{x\n\r",
		    cclass,
		    buf1,
		    buf2 );
	}

	curr_who	= (WHO_DATA *) alloc_mem( sizeof( WHO_DATA ) );
	curr_who->text	= str_dup( buf );

	if ( wch->level < LEVEL_IMMORTAL )
	{
	    if ( xIS_SET( wch->act, PLR_REGISTER ) )
	    {
    ...
    if ( first_pk )
	send_to_char( "\n\r{r---------------------------------{R{o[ {yPK CHARACTERS {r]{x{r----------------------------{x\n\r", ch );
...
void do_whois( CHAR_DATA *ch, char *argument )
{
	...
	if ( wch->level < LEVEL_IMMORTAL )
	{
	    ...
	    sprintf( buf + strlen( buf ), "{o{g%s%s%s%s{x\n\r",
		    xIS_SET( wch->act, PLR_REGISTER ) ? "(PK) "     : "",
		    xIS_SET( wch->act, PLR_KILLER   ) ? "{o{r(KILLER) {g" : "",
		    xIS_SET( wch->act, PLR_THIEF    ) ? "{o{r(THIEF) {g"  : "",
		    xIS_SET( wch->act, PLR_AFK      ) ? "{y[AFK] {g"    : "" );
	...
	else
	{
	    ...
	    sprintf( buf + strlen( buf ), "{o{g%s%s%s%s%s{x\n\r",
		    xIS_SET( wch->act, PLR_WIZINVIS ) ? "(WIZINVIS) " : "", 
		    xIS_SET( wch->act, PLR_REGISTER ) ? "(PK) "     : "",
		    xIS_SET( wch->act, PLR_KILLER   ) ? "{o{r(KILLER) {g" : "",
		    xIS_SET( wch->act, PLR_THIEF    ) ? "{o{r(THIEF) {g"  : "",
		    xIS_SET( wch->act, PLR_AFK      ) ? "{y[AFK] {g"    : "" );
...
void do_practice( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->pcdata->learned[sn] < adept )
    {
      	...
      	for ( ln = 0; ln < MAX_LANGUAGE; ln++ )
	{
	    if ( !strcmp( skill_table[sn].name, lang_table[ln].name ) )
	      	SET_BIT( ch->speaks, lang_table[ln].value );
	}
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( ch->level > 2 && !IS_SET( ch->deaf, CHANNEL_AUCTION  )
		     ? " +AUCTION"
		     : " -auction",
		     ch );
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_IMMORTAL( ch ) )
	send_to_char(  xIS_SET( ch->act, PLR_EDIT_INFO  )
	...
	send_to_char(  xIS_SET( ch->act, PLR_PAGER     )
...
void do_auction( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( time_info.hour > 18 || time_info.hour < 9 ) && !auction.item )
    {
	send_to_char( "{yThe auctioneer has retired for the evening...{x\n\r", ch );
	return;
    }
	...
	if ( !auction.item )
	{
	    send_to_char ( "{yThere is nothing being auctioned right now.  What would you like to auction?{x\n\r", ch );
	    return;
	}

	if ( auction.bet > 0 )
	    sprintf( buf, "{rCurrent bid on this item is %d gold.{x\n\r",
	    	    auction.bet );
	else
	    sprintf( buf, "{rNo bids on this item have been received.{x\n\r" );
	send_to_char( buf, ch );

	spell_identify( 0, LEVEL_HERO, ch, auction.item );

	if ( auction.item->item_type == ITEM_CONTAINER )
	{
	    act( "\n\r{o{r$p contains:{x", ch, auction.item, NULL, TO_CHAR );
	    show_list_to_char( auction.item->contains, ch, TRUE, TRUE );
	}

	if ( IS_IMMORTAL( ch ) )
	{
	    sprintf( buf, "\n\r{o{cSeller: {w%s{c.  Bidder: {w%s{c.  Round: {w%d{c.{x\n\r",
	    	    auction.seller ? auction.seller->name : "(none)",
	    	    auction.buyer  ? auction.buyer->name  : "(none)",
        	    auction.going+1 );
	    send_to_char( buf, ch );

	    sprintf( buf, "{o{cTime left in round: {w%d{c.{x\n\r", auction.pulse );
	    send_to_char( buf, ch );
	}
        return;
    }
	...
	sprintf( buf, "{o{ySale of %s has been stopped by an Immortal.{x",
	...
	sprintf( buf,
		"{o{yA bid of %d gold has been received on %s.{x",
		newbet,
		auction.item->short_descr );
        talk_auction( buf );
        return;
    ...
    if ( ( base = advatoi( arg2 ) ) <= 0 )
    {
	send_to_char( "You can't auction something for imaginary gold!\n\r", ch );
	return;
    }
    ...
    case ITEM_MONEY:
    case ITEM_PORTAL:
	act( "You cannot auction $Ts.",
				ch, NULL, item_type_name( obj ), TO_CHAR );
	break;
	...
	auction.pulse	  = PULSE_AUCTION;
	auction.going	  = 0;
	auction.starting  = base;
	auction.bet	  = base;

	sprintf( buf, "{o{yA new item is being auctioned: %s at %d gold.{x",
		obj->short_descr,
		auction.starting );
	talk_auction( buf );
	break;
