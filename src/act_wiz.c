...
void do_astat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "{cMProgs:   {x%d{c - {x%d\n\r",
	    pArea->low_mp_vnum, pArea->hi_mp_vnum );
    send_to_char( buf, ch );
    ...
    sprintf( buf, "{cFlags:    {x%s\n\r",
	    flag_string( area_flags, pArea->act ) );
    send_to_char( buf, ch );
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "{cAge: {x%d{c.  Played: {x%d{c.  Timer: {x%d{c.  Act: {x%s{c.{x\n\r",
	    get_age( victim ),
	    (int) victim->played,
	    victim->timer,
	    IS_NPC (victim) ? act_xbv (act_flags, victim->act)
			    : act_xbv (plr_flags, victim->act) );
    strcat( buf1, buf );
    ...
    resistant   = victim->resistant   | race_table[ victim->race ].resistant;
    immune      = victim->immune      | race_table[ victim->race ].immune;
    susceptible = victim->susceptible | race_table[ victim->race ].susceptible;

    sprintf( buf, "{cResistant:   {x%s{c.{x\n\r",
	    ris_bit_name( resistant ) );
    strcat( buf1, buf );
    ...
    sprintf( buf, "{cSusceptible: {x%s{c.{x\n\r",
	    ris_bit_name( susceptible ) );
    strcat( buf1, buf );
    }

    sprintf( buf, "{cAffected by: {x%s{c.{x\n\r",
	    act_xbv( affect_flags, victim->affected_by ) );
    strcat( buf1, buf );
